...
public class ManageSuperfollowServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// Account userAccount =
		// AccountDAOImplementation.getInstance().read(myusername);
		System.out.println(username);
		Account followerAccount = AccountDAOImplementation.getInstance().read(username.trim());
		...
		if (followerAccount != null) {
			if (button.equals("Accept")) {
				...
				List<Account> pending = account.getSuperFollowersPending();
				System.out.println(followerAccount.getUsername());
				for (int i = 0; i < pending.size(); i++) {
					if (pending.get(i).getUsername().equals(followerAccount.getUsername())) {
						pending.remove(i);
					}
		...
		if (updatedUserAccount != null) {
			...
			jsonObject = Json.createObjectBuilder().add("code", 200).add("moderator", jsonuser).build();
